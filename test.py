...
from langchain_experimental.autonomous_agents.autogpt import agent
...
def get_search_response(user_input, llm, memory):
    ...
    """
    initial_prompt = prompt_template("search", user_input)

    intermediate_steps_tool = Tool(
        name="Intermediate Answer",
        func=search_tool.run,
        description="Intermediate answer",
    )

    # Create a chat agent with search capabilities
    chat_agent = create_self_ask_with_search_agent(
        llm,
        [intermediate_steps_tool] + search_tools,
        prompt=initial_prompt,
    )

    agent_chain = AgentExecutor.from_agent_and_tools(
        tools=[intermediate_steps_tool] + search_tools,
        agent=chat_agent,
        llm=llm,
        verbose=False,
        memory=memory,
    ...
    try:
        ...
        formatted_scratchpad = format_agent_scratchpad(agent_chain.intermediate_steps)
        new_prompt = prompt_template(
            "search", user_input, agent_scratchpad=formatted_scratchpad
        )

        # Create a new agent with the updated prompt
        new_agent = create_self_ask_with_search_agent(
            llm,
            [intermediate_steps_tool] + search_tools,
            prompt=new_prompt,
        )

        new_agent_chain = AgentExecutor.from_agent_and_tools(
            tools=[intermediate_steps_tool] + search_tools,
            agent=new_agent,
            llm=llm,
            verbose=False,
        ...
        final_result = new_agent_chain.invoke(
            user_input, search_quality_reflection=True, search_quality_score_threshold=4
        )
        return final_result
        if "Final Answer" in result:
            response: Response = AgentOutputParser(result)
        # if "output" in result:
